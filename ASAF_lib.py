...
#------------------------------------------------------------------------------
##Ignore these old functions

def get_extrema(dirty_SNR, nside=16, npeaks=50):
    '''This function returns a nmaps x npeaks matrix. Each row is an array containing 
    the pixels with extrema SNR. Since the no. of such pixels are variable for different maps,
    I generate npeaks=50 column zero matrix, assuming that is the maximum possible no of such
    pixels. Hence, the zeros at the end each row can be ignored.'''
    nmaps = dirty_SNR.shape[0]
    npix = hp.nside2npix(nside)
    
    extr_idx = -np.ones([nmaps, npeaks]).astype('int')
    # peaks_idx[:] = -1 
    
    for map_idx in range(nmaps):
        temp_extr_idx = np.array([]).astype('int')
        
        for i in range(npix):
            
            neighbours = hp.get_all_neighbours(nside, i)
            # neighbours = hp.get_all_neighbours(nside, theta, phi)           
            neighbour_SNR = dirty_SNR[map_idx, neighbours] 
                
            for j in range(neighbours.size):
                if neighbours[j] == -1:
                    neighbour_SNR = np.delete(neighbour_SNR, neighbours[j])
        
            
            if ((dirty_SNR[map_idx ,i] > neighbour_SNR).all()) or ((dirty_SNR[map_idx ,i] < neighbour_SNR).all()):
                temp_extr_idx = np.append(temp_extr_idx, i)
                
        
        extr_idx[map_idx,0:temp_extr_idx.size] = temp_extr_idx
        # extr_idx_nzero = extr_idx[map_idx, extr_idx[map_idx]>0]
        # extr_SNR[map_idx] = dirty_SNR[map_idx, extr_idx]
    
    return extr_idx
  

def get_peaks_old(dirty_SNR, nside=16):
    '''This function returns a nmaps x npeaks matrix. Each row is an array containing 
    the pixels with maxima SNR. Since the no. of such pixels are variable for different maps,
    I generate npeaks=50 column zero matrix, assuming that is the maximum possible no of such
    pixels. Hence, the zeros at the end each row can be ignored.'''
    nmaps = dirty_SNR.shape[0]
    npix = hp.nside2npix(nside)
    ...
    neighbours_arr = []
    for i in range(npix):
        neighbours = hp.get_allneighbours(nside, i)
    ...
    for map_idx in range(nmaps):
        temp_max_idx = np.array([]).astype('int')
        
        for i in range(npix):
                      
            neighbour_SNR = dirty_SNR[map_idx, neighbours_arr[i]] 

            if ((dirty_SNR[map_idx ,i] > neighbour_SNR).all()):# or ((dirty_SNR[map_idx ,i] < neighbour_SNR).all()):
                temp_max_idx = np.append(temp_max_idx, i)
                
        
        max_idx[map_idx,0:temp_max_idx.size] = temp_max_idx
        # extr_idx_nzero = extr_idx[map_idx, extr_idx[map_idx]>0]
        # extr_SNR[map_idx] = dirty_SNR[map_idx, extr_idx]
