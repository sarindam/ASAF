...
# Some parts modified by Arindam Sharma
#-------------------------------------------------------------------------------

import sys
sys.path.append('/Users/arindam/Documents/GW_Mitra_project/ASAF/')
from ASAF_lib import get_noise_maps, inject_map2

from pystoch_full_fisher_pixel import calculate_full_fisher
...
#    csd_inj = np.dot(gamma,map_dirty_inj)*segDuration
...
# 		full_fisher_mat = []
        ...
        #nmaps = 301  #No of maps per frequency
        nmaps = len(f_all)
        print('no maps', nmaps)
        rng1 = np.random.default_rng(2)
        ninj = 4
        pix_inj = rng1.integers(0,npix, size=(nmaps, ninj) ) #np.array([400, 1072, 2098, 2987]) #rng1.integers(0, npix, size = (200,4))
		
        snr_min = 1
        snr_max = 5
        snr_inj = rng1.uniform(snr_min, snr_max, size = (nmaps,ninj))  #np.array([5,5,5,5]) * np.ones((nmaps, ninj))  #rng1.uniform(0.5, 5, size=(ninj))  
        ...
        for ll,f in enumerate(f_all):
            ...
            fisher_diag_f = np.squeeze(fisher_diag_f)
			
            full_fisher = calculate_full_fisher(f, frame_param.segDuration, csd[ll:ll+1], sigma_sq_inv[ll:ll+1], combined_antenna_response, t_delay, H[ll], notch_array[ll]) 
            dirty_SNR_inj, noise_SNR = inject_map2(full_fisher, fisher_diag_f, pix_inj[ll,:], snr_inj[ll,:], 1, ret_noise=True) 
            
            # injected_dirty_map_mat.append(injected_dirty_map)
            dirty_SNR_inj_mat.append(dirty_SNR_inj)
            noise_SNR_mat.append(noise_SNR)
			map_dirty_mat.append(map_dirty_f)
            fisher_diag_mat.append(fisher_diag_f)
			#full_fisher_mat.append(full_fisher)
            snr_inj_mat.append(snr_inj)
...
#     fisher_diag = np.sum(fisher_diag_h, axis=0)
    ...
    maps_mat_output = h5py.File(parameters.output_map_path + 'Maps_'+dataset+'_'+str(parameters.nside)+ '_'+str(parameters.f_min)+ '-' + str(parameters.f_max) + 'Hz_step' + str(frame_param.deltaF)+ 'Hz_'+str(ninj)+ 'inj_snrinj' + str(snr_min) + '-' + str(snr_max) +  '.hdf5', "w")
   ...
   # maps_mat_output.create_dataset('injected_dirty_map', data = injected_dirty_map_mat)
    maps_mat_output.create_dataset('dirty_SNR_inj', data = dirty_SNR_inj_mat)
    maps_mat_output.create_dataset('noise_SNR', data = noise_SNR_mat)
    maps_mat_output.create_dataset('pix_inj', data = pix_inj)
    maps_mat_output.create_dataset('snr_inj', data = snr_inj_mat)
    #maps_mat_output.create_dataset('fisher_full', data = full_fisher)
