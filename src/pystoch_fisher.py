...
def inject_map(fisher_full, fisher_diag, pix_inj, snr_inj, nside):
    
    #n is the npix X 1 array containing noise realisations
    npix = hp.nside2npix(nside)
    mean_arr = np.zeros(npix)
    rng = np.random.default_rng(1)
    n = rng.multivariate_normal(mean_arr, fisher_full.real)
    
    #print('n shape =', n.shape)
    #Phat is the vector containing only the SNR of the injected pixel in the 
    #location where the pixel is injected. Everything else is zero.
    Phat = np.zeros(npix)
    
    for i, p in enumerate(pix_inj):
        Phat[p] = snr_inj[i]
    
    Phat = Phat/np.sqrt(fisher_diag)
      
    X = np.dot(fisher_full, Phat.T) + n
    dirty_SNR = X/np.sqrt(fisher_diag)
    return X, dirty_SNR
...
# 		full_fisher_mat = []
        ...
        #pix_inj = rng1.integers(0, npix, size = (200,4))
        #snr_inj = rng1.uniform(10,15,size = (200,4))
        #for ll,f in enumerate(f_all):
            ...
            #injected_dirty_map, dirty_SNR_inj = inject_map(full_fisher, fisher_diag_f, pix_inj[ll], snr_inj[ll], nside=parameters.nside) 
...
#     fisher_diag = np.sum(fisher_diag_h, axis=0)
    ...
    maps_mat_output = h5py.File(parameters.output_map_path + 'Fisher_full_'+dataset+'_'+str(parameters.nside)+ '_'+str(f)+'Hz.hdf5', "w")
    ...
    #maps_mat_output.create_dataset('snr_inj', data = snr_inj)
