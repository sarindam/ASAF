...
for dataset in datasets:
    ...
    # in case of no multiprocessing
        ...
        ffisher_broad = np.zeros((npix, npix))
        notch_array = make_notch_array(f_all,parameters.notching,parameters.notch_list)
        for ll,f in enumerate(f_all):

            ffisher_broad = ffisher_broad + calculate_full_fisher(f, frame_param.segDuration, csd[ll:ll+1], sigma_sq_inv[ll:ll+1], combined_antenna_response, t_delay, H[ll], notch_array[ll])           
...
#     fisher_diag = np.sum(fisher_diag_h, axis=0)
    ...
    maps_mat_output = h5py.File(parameters.output_map_path + 'FFisher_broad_'+dataset+'_'+str(parameters.nside)+'_' + str(parameters.f_min) + '-' + str(parameters.f_max) + 'Hz.hdf5', "w")
    #maps_mat_output.create_dataset('map_dirty', data = map_dirty_mat) 
    maps_mat_output.create_dataset('ffisher_broad', data= ffisher_broad)
